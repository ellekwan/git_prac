...
def row_dict_values():
    ...
    try:
        ...
        data_rng = sheet1[head_data:cellmax]
        print(co)
...
## add in names to ignore
            if name != None and \
            "N/A" not in name and "Provisioning" not in name and \
            "Example" not in name and \
...
def dict_tab_values():
    ...
    print("Creating csv file {0}\n...".format(csv_filename))
    wr = csv.writer(csvfile, quoting=csv.QUOTE_ALL)
    cursorA = conn.cursor()
    print("Using database users_info_test")
...
#!csv import headers should be 'USER NAME', 'IDENTITY', 'EMAIL', 'PHONE NUMBER' 'PROVISION' ??
